...
def main():
    ...
    #print("Lora_peft_layers",Lora_peft_layers)
    
    model=LORA_peft(model,Lora_peft_layer_name_pre,Lora_peft_layer_name_suffix,LP_layer_no_start,LP_layer_no_end)
    model=model.to(device)
    '''layers=["head.up1.1","head.up1.2","head.up2.1","head.up2.2","head.up3.1","head.up3.2","head.up4.1",
            "head.up4.2","head.up5.0","head.up5.1"]'''
    layers=config["no_LORA_trainable_layers"]

    
    for name, param in model.named_parameters():
        #print("name",name)
        layer_name=".".join(name.split(".")[2:5])    
        if layer_name in layers:
            param.requires_grad = True  # Enable gradient computation for these layers
