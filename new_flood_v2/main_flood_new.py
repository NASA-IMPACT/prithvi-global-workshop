...
from data_load_flood_new import flood_dataset
...
def segmentation_loss(mask,pred,device,class_weights,ignore_index):
    ...
    mask=mask.long()
    pred=pred.squeeze(2) #time dim =1, squeezed
...
def compute_accuracy(labels, output):
    ...
    output=output.squeeze(2) #time dim =1, squeezed
...
def plot_output_image(model, device, epoch,means,stds,input_path,prediction_img_dir):
    ...
    with torch.no_grad():
        output = model(final_image)  # [1, n_segmentation_class, 1, 224, 224]

    # Remove batch dimension
    output = output.squeeze(0)  #[n_segmentation_class,1, 224, 224]
    output = output.squeeze(1) #[n_segmentation_class,224, 224]

    # Convert output to predicted class
    predicted_mask = torch.argmax(output, dim=0)  # [224, 224]
...
def calculate_miou(output, target, device):
    ...
    eps=1e-6
    output=output.squeeze(2) #time dim =1, squeezed # b,n_class,224,224
    num_classes=output.shape[1]
    preds = torch.argmax(output, dim=1) # b,224,224
...
def main():
    ...
    test_annot=config["data"]["test_annot"]     
    ...
    path_test=data_path("testing",data_dir_input,data_dir_mask,test_annot)
    flood_dataset_train=flood_dataset(path_train,means,stds)
    flood_dataset_val=flood_dataset(path_val,means,stds)
    flood_dataset_test=flood_dataset(path_test,means,stds)
    ...
    test_dataloader=DataLoader(flood_dataset_test,batch_size=test_batch_size,shuffle=config["testing"]["shuffle"],num_workers=1)
    ...
    for i in range(n_iteration):
        ...
        with torch.no_grad():
            for j,(input,mask) in enumerate(test_dataloader):
                ...
                acc_dataset_test.append(batch_acc)
                test_loss += loss.item() * input.size(0) 

                miou_batch=calculate_miou(out, mask, device)
                miou_test.append(miou_batch)  
    
        acc_total_test=np.mean(acc_dataset_test)
        epoch_loss_test = test_loss / len(test_dataloader.dataset)
        miou_test=np.mean(miou_test)

        wandb.log({"epoch": i + 1, "test_loss": epoch_loss_test,"accuracy_test":acc_total_test,"miou_test":miou_test})
        
        print(f"Epoch: {i}, train loss: {epoch_loss_train}, val loss:{epoch_loss_val},accuracy_train:{acc_total_train},accuracy_val:{acc_total_val},accuracy_test:{acc_total_test},miou_val:{miou_valid},miou_test:{miou_test}")
