...
def lr_lambda(current_step):
    if current_step < warmup_iters:
        # Linear warmup phase
        lr=start_lr + (max_lr - start_lr) * (current_step / warmup_iters)
        #print("current step, lr",current_step,lr)
        #print("current step,lr: ",current_step,lr)
        return lr

    else:
        # Polynomial decay
        #print("decreasing current step, lr: ",current_step, (1 - (current_step - warmup_iters) / (total_steps - warmup_iters)) ** power )
        lr=max_lr-(max_lr-final_lr)*((current_step-warmup_iters)/(total_steps-warmup_iters))
        #print("after warmup lr:", lr)
        return lr
