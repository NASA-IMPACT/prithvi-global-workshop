...
def print_trainable_layers(model: torch.nn.Module):
    """
    Prints out the layers in the model and specifies whether they are trainable.
    """
    print("\nTrainable layers:")
...
def main():
    ...
    #used LORA for prithvi model only: line 73, prithvi_global_loader.py
    model=prithvi_wrapper(config,n_channel,n_class,n_frame,embed_size,input_size,patch_size,config["prithvi_model_new_weight"],config["prithvi_model_new_config"]) #wrapper of prithvi #initialization of prithvi is done by initializing prithvi_loader.py
    model=model.to(device)
    
    # LORA cannot train BatchNormlayers in segmentation head, 
    ## so, all segmentation head params are set to be separately trainable without LORA

    print_trainable_parameters(model=model)
    print_trainable_layers(model)
    print("Note: decoder layers are loaded but not used during forward operation of the model ")
